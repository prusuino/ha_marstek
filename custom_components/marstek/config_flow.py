...
class MarstekConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _entry_matches_mac(
        self, entry: ConfigEntry[Any], discovered_mac: str
    ) -> bool:
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if unique_id_mac:
                normalized_mac = self._normalized_mac(unique_id_mac)
                if normalized_mac:
    ...
    async def _discover_devices_with_retry(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                last_error = err
                _LOGGER.error("Device discovery failed, attempt %d: %s", attempt, err)
                ...
                if attempt == max_retries:
                    ...
                    cache = getattr(udp_client, "_discovery_cache", None)
                    if cache:
    ...
    async def async_step_integration_discovery(
        ...
        normalized_mac = self._normalized_mac(discovered_ble_mac)
    ...
    async def _async_handle_discovery_with_unique_id(
        ...
        for entry in self._async_current_entries(include_ignore=False):
            ...
            if entry.data.get(CONF_HOST) != self._discovered_ip:
                ...
                self.hass.config_entries.async_update_entry(
                    entry,
                    data={**entry.data, CONF_HOST: self._discovered_ip},
                )
